...
export class QuotationController {
  ...
  generateQuotationSupplyLaborDocxV2 = async (req: Request, res: Response): Promise<void> => {
    const request = schemas.QuotationSchema.safeParse(req.params);
    if (!request.success) {
      res.status(StatusCode.BadRequest).json({
        status: StatusValue.Failed,
    ...
    const response = await this.quotationService.generateQuotationSupplyLaborDocxV2(request.data.idQuotation);
    if (Buffer.isBuffer(response)) {
      res.set({
        "Content-Type": "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        "Content-Disposition": "attachment; filename=\"COT_SACIPR_Suministro_Mano_Obra_V2_Generado.docx\"",
      });
      res.send(response);
      return;
    }
