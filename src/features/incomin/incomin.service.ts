...
import { IncominRepository } from "./incomin.repository";
...
export class IncominService {

    private readonly expenditureRepository: IncominRepository;

    constructor(expenditureRepository: IncominRepository) {
    ...
    //     try {
    //         const expenditure = await this.expenditureRepository.findById(idIncome);
    //         if (!expenditure) {
    ...
    findById = async (idIncome: number): Promise<ResponseEntity> => {
        try {
            ...
            // const filter = { idExpenditure: expenditure.idIncome };
            // const expenditureItems = await this.expenditureRepository.findAllExpenditureItem(-1, 0, filter);
    ...
    update = async (request: dtos.UpdateDTO, filePath?: string): Promise<ResponseEntity> => {
        try {
            const expenditure = await this.expenditureRepository.findById(request.idIncome);
            if (!expenditure) {
                return BuildResponse.buildErrorResponse(StatusCode.NotFound, { message: "Expenditure not found" });
            }
            ...
            expenditure.idInvoice = request.idInvoice || expenditure.idInvoice;
