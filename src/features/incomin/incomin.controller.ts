...
import { IncominService } from "./incomin.service";
import * as schemas from "./incomin.schema";
import { Request, Response } from "express";
import { StatusCode, StatusValue } from "../../utils/general.interfase";
import { formatZodError } from "../employee/utils";
import { UploadedFile } from "express-fileupload";

export class IncominController {
  private readonly expenditureService: IncominService;

  constructor(expenditureService: IncominService) {
  ...
  findById = async (req: Request, res: Response): Promise<void> => {
    ...
    const response = await this.expenditureService.findById(request.data.idIncome);
    res.status(response.code).json({
      status: response.status,
      data: response.data
    });
  ...
  delete = async (req: Request, res: Response): Promise<void> => {
    ...
    const response = await this.expenditureService.delete(request.data.idIncome);
    res.status(response.code).json({
      status: response.status,
      data: response.data
    });
