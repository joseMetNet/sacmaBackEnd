...
export class ReportsController {
  ...
  getReportExpenditureIncomeInvoice = async (req: Request, res: Response): Promise<void> => {
    const request = schemas.getReportExpenditureIncomeInvoice.safeParse(req.query);
    if (!request.success) {
      res.status(StatusCode.BadRequest).json({
        status: StatusValue.Failed,
        data: { error: formatZodError(request.error) },
